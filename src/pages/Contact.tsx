        ...
        <div className="section-shell grid gap-8 lg:grid-cols-[1fr_0.85fr]">
          <div className="grid gap-4 md:grid-cols-2">
            {channels.map(({ label, value, Icon }) => (
              ...
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#fbf6ec] text-[#7a5520]">
                  <Icon size={24} weight="duotone" />
                </div>
              <h2 className="mt-6 text-xl font-black text-[#242321]">{label}</h2>
                <p className="mt-3 text-sm leading-6 text-slate-600">{value}</p>
              </div>
            ))}
          </div>
          <div className="glass-panel rounded-2xl p-8">
            <h2 className="text-4xl font-black tracking-normal text-[#242321]">Ready to send project details?</h2>
