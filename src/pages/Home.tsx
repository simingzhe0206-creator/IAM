  ...
  return (
    <>
      <section className="py-8 md:py-12">
        <div className="section-shell">
          <div className="page-canvas overflow-hidden p-5 md:p-8">
            <div className="relative min-h-[520px] overflow-hidden rounded-2xl bg-[#f2e6cf]">
              <img className="absolute inset-0 h-full w-full object-cover image-treatment" src={assets.hero} alt="" />
              <div className="absolute inset-0 bg-gradient-to-r from-[#242321]/82 via-[#242321]/48 to-[#bd8b36]/18" />
              <div className="absolute inset-0 bg-gradient-to-t from-[#242321]/20 via-transparent to-white/6" />
            ...
            <div className="grid gap-10 py-14 md:py-16 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
              <div>
                <h2 className="text-balance text-4xl font-black tracking-normal text-[#242321] md:text-5xl">
                  About IAM
                </h2>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {stats.slice(0, 4).map(([value, label]) => (
                  <div key={label} className="min-w-0 rounded-2xl bg-[#fbf6ec] p-5">
                    <div className="text-3xl font-black tracking-normal text-[#242321]">{value}</div>
                    <div className="mt-2 break-words text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">{label}</div>
                  </div>
                ))}
              </div>
            </div>

            <div className="grid gap-3 border-y border-slate-200 py-5 md:grid-cols-5">
              {audience.map((item) => (
                <div key={item} className="rounded-xl bg-white px-4 py-3 text-center text-sm font-bold text-slate-500">
                  {item}
                </div>
              ))}
            </div>
        ...
        <div className="section-shell">
          <div className="page-canvas grid gap-8 overflow-hidden p-5 md:p-8 lg:grid-cols-[0.9fr_1.1fr]">
            <div className="relative min-h-[520px] overflow-hidden rounded-2xl bg-[#f2e6cf]">
              ...
              <div className="absolute right-6 top-6 flex h-32 w-32 items-center justify-center rounded-full bg-[#bd8b36] text-center text-sm font-black leading-5 text-[#242321] shadow-[0_18px_42px_rgba(36,35,33,0.24)]">
                Clear quote
                <br />
                process
              </div>
            ...
            <div className="flex flex-col justify-center py-4">
              ...
              <div className="mt-8 grid gap-4">
                {steps.map(({ title, text, Icon }, index) => (
                  <div key={title} className="grid grid-cols-[3rem_1fr] gap-4 rounded-2xl border border-slate-200 bg-white p-4">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#242321] text-[#d2a253]">
                      <Icon size={22} weight="duotone" />
                    </div>
                    <div>
                      <div className="text-xs font-black uppercase tracking-[0.12em] text-slate-400">Step {index + 1}</div>
                      <h3 className="mt-1 text-lg font-black text-[#242321]">{title}</h3>
              ...
              {projectTypes.map((project, index) => (
                ...
                >
                  <img
                    ...
                    alt=""
                  />
                  <div className="absolute inset-0 bg-gradient-to-br from-white/94 via-[#fffdfa]/76 to-[#f2e6cf]/56" />
                  <div className="relative z-10 flex h-full flex-col justify-end">
                    <h3 className="max-w-xl text-2xl font-black tracking-normal text-[#242321]">{project.title}</h3>
                    <p className="mt-3 max-w-xl text-sm leading-6 text-slate-600">{project.description}</p>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <section className="py-8 pb-14 md:py-12 md:pb-20">
        <div className="section-shell">
          <div className="rounded-2xl bg-[#242321] p-8 text-white shadow-[0_24px_70px_rgba(0,0,0,0.30)] md:p-12">
            <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
              ...
              <Link
                to="/quote"
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#bd8b36] px-6 text-sm font-black text-white transition hover:bg-[#a9782e]"
              >
                Request a Quote
